...
| Airports by current code, previous code as alias | `EuroAipModel.find_airport_by_code`: exact `ident`, then `alt_ident`; resolved airports are named by `ident` | OurAirports' `ident` is a placeholder for some aerodromes (Enstone `GB-0007`, ICAO EGTN) and a superseded code for others (Logroño `LELO`, AIP code LERJ). Airports are stored under the current code (`euro_aip.utils.assign_airport_codes`) and the superseded one kept as `alt_ident`, so a pilot typing LELO gets LERJ. |
...
- **Inline ICAO coordinates resolve geometrically**: Tokens like `4830N00210E` (DM, 11 chars) and `483012N0021034E` (DMS, 15 chars) classify as `TokenKind.COORDINATE` and resolve to a `RoutePoint(point_type="coordinate")` via `parse_icao_coordinate` — no DB lookup. The token itself becomes the point name so it round-trips back into the route string verbatim. Slash-separated forms like `4830N/00210E` still split on `/` (parser treats it as `4830N` + qualifier `00210E`) — those stay UNKNOWN; use the unslashed form for inline coords.
- **A superseded airport code is an alias, not a second airport**: `find_airport_by_code("LELO")` returns the LERJ airport, and the resolver names it `LERJ` in `Route.departure`/`waypoints`. Callers matching resolved names back to typed tokens must compare through `find_airport_by_code`, not string equality. `airports.get()`/`airports[...]` stay exact on purpose. The alias matters beyond typing: Logroño's METAR/TAF are still issued as LELO, so `RouteWeatherService` requests both codes and files reports under `ident`.
